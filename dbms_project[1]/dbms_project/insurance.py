...
end_date_entry= DateEntry(win)

end_date=StringVar()


end_date_entry.config(textvariable=end_date)
    ...
    if e1.get() == "" or e5.get() == "":
        ...
        try:
            with engine.connect() as conn:
                # Get the date from the DateEntry widget and format it as a string
                ...
                conn.execute(
                    text("INSERT INTO insurance (policy_number, patient_id, ins_code, end_date, provider) VALUES (:p_n, :p_id, :i_c, :e_d, :pro)"),
                    {
                        ...
                        'p_id': patient_id.get(),
                        'i_c': ins_code.get(),
                        'e_d': end_date_str,  # Use the formatted date string
                        'pro': provider.get(),
                    }
                )
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            for item in rows:
                ...
                if isinstance(item[3], datetime):  # Check if it's a datetime object
...
table.heading('i_c',text='ind_code')
