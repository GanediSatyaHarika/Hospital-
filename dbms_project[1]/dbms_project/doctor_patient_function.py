...
def save():
    try:
        doc_id = e1.get()  # Assuming e1 is the Entry widget for doctor ID
...
def fetch_data(doc_id):
    try:
        stmnt = 'SELECT * FROM view_doctor_patient_details(:doc_id)'
        values = {'doc_id': doc_id}

        result = db.execute_dql_commands(stmnt, values=values)
        
        # Check if result is None or has valid data
        if result is not None:
            ...
            for item in rows:
                ...
                item[13] = item[13].strftime('%Y-%m-%d ')
                item[14] = item[14].strftime('%Y-%m-%d ')
                table.insert('', END, values=item)
        else:
            print("No data returned from database.")  # Debug statement
    except Exception as err:
        messagebox.showerror('Error', f'Failed to fetch data: {err}')
        print(f'Error in fetch_data: {err}')  # Debug statement
...
table = ttk.Treeview(frame2, columns=('doc_id','d_fn','d_ln','qua','spa','p_id','p_fn','p_ln','ph','add','age','b_t','gen','adm_d','dis_d','pre_id','m_n','dose','pre_d','b_id','b_d','t_c'),xscrollcommand=scroll_y.set,yscrollcommand=scroll_x.set )
...
table.heading('spa',text='specialization')
