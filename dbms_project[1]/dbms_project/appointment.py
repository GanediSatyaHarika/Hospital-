...
Scheduled_on_entry= DateEntry(win)

Scheduled_on=StringVar()


Scheduled_on_entry.config(textvariable=Scheduled_on)
    ...
    if e1.get() == "" or e5.get() == "":
        ...
        try:
            with engine.connect() as conn:
                # Get the time entered by the user from the Entry widget
                entered_time = e4.get()
                # Validate and format the time if it's in a valid format
                try:
                    formatted_time = datetime.strptime(entered_time, '%H:%M:%S').strftime('%H:%M:%S')
                except ValueError:
                    messagebox.showerror("Error", "Invalid time format. Please enter time in HH:MM:SS format.")
                    return
                
                conn.execute(
                    text("INSERT INTO appointment (Appt_id, Scheduled_on, date, time, doctor_id, patient_id) VALUES (:a_id, :s_o, :date, :time, :doc_id, :p_id)"),
                    {
                        'a_id': appt_id.get(),
