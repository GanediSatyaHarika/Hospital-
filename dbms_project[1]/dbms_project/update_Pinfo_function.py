...
def fetch_data(p_id, ph, add):
    try:
        stmnt = 'SELECT * FROM UpdatePatientInfoFunction(:p_id, :ph, :add)'
        with engine.connect() as conn:
            result = conn.execute(text(stmnt), {'p_id': p_id, 'ph': ph, 'add': add})
...
Label(lf1,text='address',font='ariel 15',bg='pink').place(x=5,y=150)
...
e3.config(textvariable=address)
