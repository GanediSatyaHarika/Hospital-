...
def save():
    if e1.get() == "":
        ...
        try:
            ...
            app_date=e3.get()
            app_time=e4.get()
            fetch_data(p_id,doc_id,app_date,app_time)  
...
def fetch_data(p_id, doc_id, app_date, app_time):
    try:
        stmnt = 'SELECT * FROM scheduleappointment(:p_id,:doc_id,:app_date,:app_time)'
        
        with engine.connect() as conn:
            result = conn.execute(text(stmnt), {'p_id': p_id, 'doc_id': doc_id, 'app_date': app_date, 'app_time': app_time})
...
Label(lf1,text='app_date',font='ariel 15',bg='pink').place(x=5,y=150)
Label(lf1,text='app_time',font='ariel 15',bg='pink').place(x=5,y=200)
...
e3.config(textvariable=app_date)

e4 = Entry(lf1, bd=4)
e4.place(x=200, y=200, width=200)
e4.config(textvariable=app_time)
